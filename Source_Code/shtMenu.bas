...
Function fOpenFileSelectDialogAndSetToSheetRangeForCompany(sCompany As String)
    ...
    sHeader = LeftB(shtMenu.Range("rngHeader_" & sCompany).Value, LenB(shtMenu.Range("rngHeader_" & sCompany).Value) - 2)
    Call fOpenFileSelectDialogAndSetToSheetRange("rngSalesFilePath_" & sCompany, , sHeader)
End Function
