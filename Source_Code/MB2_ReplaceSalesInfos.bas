...
Sub subMain_ReplaceSalesInfos()
    ...
    Call fSetReplaceUnifyErrorRowCount_SCompSalesInfo(999)
...
error_handling:
    ...
    If Err.Number <> 0 And gErrNum = 0 Then
        gErrNum = Err.Number
        gsBusinessErrorMsg = Err.Description
    Else
        ...
        '    Call fSortDataInSheetSortSheetData(shtSalesRawDataRpt, Array(dictRptColIndex("SalesCompanyName") _
           ...
           ' Call fProtectSheetAndAllowEdit(shtSalesRawDataRpt, shtSalesRawDataRpt.Columns(4), UBound(arrOutput, 1) + 1, UBound(arrOutput, 2), False)
            ...
            Call fSetFormatForExceptionCells(shtSalesInfos, dictErrorRows, "REPORT_ERROR_COLOR")
            Call fSetFormatForExceptionCells(shtSalesInfos, dictWarningRows, "REPORT_WARNING_COLOR")
            Call fSetReplaceUnifyErrorRowCount_SCompSalesInfo(dictErrorRows.Count)
...
'            arrWarningRows(alWarningCnt) = dictRptColIndex("Hospital")
            Call fAddWarningColumnTodictWarningRows(lEachRow + 1, dictRptColIndex("Hospital"))
        End If
